...
- 玩家一号 / 人机对战 / 在线联机：`WASD` 移动，鼠标移动准心，`1-5` 切换武器槽，`空格` 或鼠标左键执行当前武器；槽位为 `1` 主武器、`2` 雷管、`3` 滚珠轴承、`4` 远程星链、`5` 远程防空。`Q` / 右键、`R`、`E`、`F` 仍保留为直接快捷键。
- 本地双人玩家二号：方向键移动，`6-0` 切换同款武器槽，`Enter` 执行当前武器；`/`、`.`、小键盘 `9/0` 仍保留为直接快捷键。
