...
新版画面加入程序化战术服贴图、人形战员、弹坑、跑道灯、远景山脊、星空、烟尘和多层战斗灯光；战斗地图也扩大为超过原可玩面积 5 倍的开阔星链靶场。

## 模式

- 本地双人：同屏两名玩家对战
- 人机对战：玩家一号对抗 AI 战员
- 在线联机 / PK：登录账号后进入，使用与人机对战一致的本机快捷键
- 机机对战：两名 AI 自动决斗
- 基地规则：每方基地都有一台坏电脑，敌方进入基地并修好坏电脑会直接获胜
- 军用导航：战员必须在己方基地内才会接入基地雷达链路，雷达只标记未被建筑、掩体和物资遮挡的目标
- 加固建筑：地图新增指挥楼、雷达站、装甲库、维修库等实体建筑，可遮挡基地导航、吸收子弹，并削弱雷管爆炸冲击
- 烧开水补能：移动和开枪都会消耗能量，只有回到己方基地烧开水才能恢复能量
- 近防炮：基地内可手动瞄准近防炮压制，理论射速 13000 发/分钟，同样消耗能量
- 防空武器：对方星链来袭时可发射拦截弹，视角会切换到弹体，命中在途星链即可拦截
- 星链模式：玩家可在发射前选择自动或操控；自动星链沿固定弹道砸向目标，操控星链会切入弹体视角，可转向躲避防空武器并手动砸中敌方基地
- 雷管：可发射高耗能雷管，命中或引信结束后爆炸，把范围内对手炸飞
...
- 玩家一号 / 人机对战 / 在线联机：`WASD` 移动，鼠标移动准心，`空格` 或鼠标左键开火，`Q` 或鼠标右键发射雷管，`T` 切换星链自动/操控，`E` 呼叫星链，`F` 发射防空武器
- 本地双人玩家二号：方向键移动，`Enter` 开火，`/` 发射雷管，`9` 切换星链自动/操控，`.` 呼叫星链，`0` 发射防空武器
